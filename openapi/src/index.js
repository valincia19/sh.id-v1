...
app.use(
    cors({
        // By allowing '*', we CANNOT use credentials: true as that violates modern security norms & CSRF guarantees
        origin: "*",
...
const globalLimiter = rateLimit({
    ...
    legacyHeaders: false,
    passOnStoreError: true, // Fail-open: allow requests if Redis crashes
    message: {
...
app.use((err, req, res, next) => {
    if (config.isDevelopment) {
        ...
        // Obfuscate underlying stack trace completely in production log output
        logger.error(`Error: ${err.message || 'Unknown Error'} - Endpoint: ${req.originalUrl} - IP: ${req.ip}`);
    }
