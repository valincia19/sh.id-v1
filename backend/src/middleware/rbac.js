...
export const loadUserPermissions = async (userId) => {
  try {
    ...
    if (permissions) {
      if (config.isDevelopment) logger.debug(`[RBAC] Permissions CACHE HIT for ${userId}`);
      return permissions;
    }

    if (config.isDevelopment) logger.debug(`[RBAC] Permissions CACHE MISS for ${userId} - Fetching from DB`);
...
/**
 * Load user roles from cache or database
 * @param {string} userId - User ID
...
export const loadUserRoles = async (userId) => {
  try {
    // Try cache first
    let roles = await getCachedUserRoles(userId);
    if (roles) {
      if (config.isDevelopment) logger.debug(`[RBAC] Roles CACHE HIT for ${userId}`);
      return roles;
    }

    if (config.isDevelopment) logger.debug(`[RBAC] Roles CACHE MISS for ${userId} - Fetching from DB`);
