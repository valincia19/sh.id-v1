  ...
  body("password")
    ...
    .custom((value) => {
      ...
      if (commonPatterns.some(pattern => pattern.test(value))) {
        throw new Error("Password is too common or easily guessed");
      }
      return true;
    }),
  body("displayName")
    ...
    .custom((value) => {
      // Check for XSS patterns after sanitization
      const xssPatterns = ['<svg', '<script', 'javascript:', 'onerror=', 'onload='];
...
export const login = async (req, res) => {
  try {
    ...
    res.cookie("accessToken", result.tokens.accessToken, {
      ...cookieOptions,
      maxAge: 24 * 60 * 60 * 1000, // 24 hours (actual expiry handled by JWT)
    });
