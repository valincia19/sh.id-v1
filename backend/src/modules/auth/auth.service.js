...
export const registerUser = async (userData) => {
  ...
  try {
    return await transaction(async (client) => {
      ...
      // Hash password
      logger.debug('Hashing password for registration (len: %d)', password?.length);
      const passwordHash = await bcrypt.hash(
        password,
        config.security.bcryptRounds
      );
      logger.debug('Password hashed successfully (hash len: %d)', passwordHash?.length);
 ...
 */
export const loginUser = async (identifier, password, ipAddress = 'unknown') => {
  try {
    ...
    // ============================================
    const lockStatus = await isAccountLocked(identifier, ipAddress);
    if (lockStatus.isLocked) {
      const minutesRemaining = Math.ceil(lockStatus.remainingTime / 60);
      throw {
        statusCode: 423, // Locked
        message: `Account is temporarily locked due to too many failed login attempts. Please try again in ${minutesRemaining} minute${minutesRemaining !== 1 ? 's' : ''}.`,
        code: 'ACCOUNT_LOCKED',
    ...
    logger.debug('User lookup result: %d rows for identifier: %s', userResult.rows.length, identifier);
    ...
    const user = userResult.rows[0];
    logger.debug('Found user: id=%s, username=%s, hasPassword=%s', user.id, user.username, !!user.password_hash);
    ...
    // ============================================
    // Step 5: Check if password exists (social login)
    // ============================================
    ...
    // ============================================
    logger.debug('Comparing passwords for user %s (input len: %d, hash len: %d)',
      user.id, password?.length, user.password_hash?.length);
    const isPasswordValid = await bcrypt.compare(password, user.password_hash);
    logger.debug('Password valid: %s', isPasswordValid);

    if (!isPasswordValid) {
      // Record failed attempt
      const attemptResult = await recordFailedAttempt(identifier, ipAddress);
    ...
    // ============================================
    // Step 7: Clear failed attempts on success
    // ============================================
    await clearFailedAttempts(identifier, ipAddress);
