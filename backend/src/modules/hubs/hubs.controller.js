...
export const getHubBySlug = async (req, res) => {
    try {
        ...
        const isOwner = req.user?.userId && hub.owner_id === req.user.userId;
        const allowedStatuses = isAdmin || isOwner
            ? ["active", "suspended", "review", "pending"]
            : ["active", "suspended"];

        if (!allowedStatuses.includes(hub.status)) {
            return res.status(404).json({
                error: "NotFound",
