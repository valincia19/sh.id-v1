...
export const getAdminScripts = async ({ limit = 50, offset = 0, search = '', status = '' } = {}) => {
    ...
    const statusParam = isDeletedFilter ? '' : status;
    ...
    const query = `
        ...
        WHERE ${isDeletedFilter ? 's.deleted_at IS NOT NULL' : 's.deleted_at IS NULL'}
    ...
    `;
    const [rows, countRow] = await Promise.all([
        pool.query(query, [limit, offset, searchParam, statusParam]),
        pool.query(countQuery, [searchParam, statusParam]),
    ]);
...
export const deleteScript = async (scriptId) => {
    const result = await pool.query(
        `UPDATE scripts SET deleted_at = NOW() WHERE id = $1 AND deleted_at IS NULL RETURNING id`,
        [scriptId]
    );
 ...
 */
export const restoreScript = async (scriptId) => {
    const result = await pool.query(
        `UPDATE scripts SET deleted_at = NULL, updated_at = NOW() WHERE id = $1 AND deleted_at IS NOT NULL RETURNING id`,
        [scriptId]
    );
    if (result.rowCount === 0) throw { statusCode: 404, message: 'Script not found or not deleted' };
...
export const getAdminExecutors = async ({ limit = 50, offset = 0, search = '', status = '' } = {}) => {
    ...
    const rows = await pool.query(`
        SELECT
            e.id, e.name, e.slug, e.status, e.created_at, e.price_model, e.deleted_at,
            u.username AS owner_username,
            (SELECT COUNT(*) FROM executor_versions v WHERE v.executor_id = e.id)::int AS version_count
        FROM executors e
        LEFT JOIN users u ON e.owner_id = u.id
        WHERE ${isDeletedFilter ? 'e.deleted_at IS NOT NULL' : 'e.deleted_at IS NULL'}
          AND ($3 = '' OR e.name ILIKE $3 OR e.slug ILIKE $3 OR u.username ILIKE $3)
          AND ($4 = '' OR e.status::text = $4)
        ORDER BY e.created_at DESC
        LIMIT $1 OFFSET $2
    `, [limit, offset, s, statusParam]);
    const countRow = await pool.query(`
        SELECT COUNT(*)::int AS total
        FROM executors e LEFT JOIN users u ON e.owner_id = u.id
        WHERE ${isDeletedFilter ? 'e.deleted_at IS NOT NULL' : 'e.deleted_at IS NULL'}
          AND ($1 = '' OR e.name ILIKE $1 OR e.slug ILIKE $1 OR u.username ILIKE $1)
...
/**
 * Soft-delete an executor
 */
export const deleteExecutor = async (executorId) => {
    const result = await pool.query(
        `UPDATE executors SET deleted_at = NOW() WHERE id = $1 AND deleted_at IS NULL RETURNING id`,
        [executorId]
    );
    if (result.rowCount === 0) throw { statusCode: 404, message: 'Executor not found or already deleted' };
    return { deleted: true };
};

/**
 * Restore a soft-deleted executor
 */
export const restoreExecutor = async (executorId) => {
    const result = await pool.query(
        `UPDATE executors SET deleted_at = NULL, updated_at = NOW() WHERE id = $1 AND deleted_at IS NOT NULL RETURNING id`,
        [executorId]
    );
    if (result.rowCount === 0) throw { statusCode: 404, message: 'Executor not found or not deleted' };
    return { restored: true };
};
