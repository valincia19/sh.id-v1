...
/**
 * PATCH /api/admin/scripts/:id/restore
 */
export const restoreScript = async (req, res) => {
    try {
        const result = await adminService.restoreScript(req.params.id);
        res.json({ success: true, data: result });
    } catch (error) {
        const code = error.statusCode || 500;
        res.status(code).json({ error: 'Error', message: error.message || 'Failed to restore script' });
    }
};
 ...
 */
export const deleteExecutor = async (req, res) => {
    try {
        const result = await adminService.deleteExecutor(req.params.id);
        res.json({ success: true, data: result });
    } catch (error) {
        const code = error.statusCode || 500;
        res.status(code).json({ error: 'Error', message: error.message || 'Failed to delete executor' });
    }
};

/**
 * PATCH /api/admin/executors/:id/restore
 */
export const restoreExecutor = async (req, res) => {
    try {
        const result = await adminService.restoreExecutor(req.params.id);
        res.json({ success: true, data: result });
    } catch (error) {
        const code = error.statusCode || 500;
        res.status(code).json({ error: 'Error', message: error.message || 'Failed to restore executor' });
    }
};
