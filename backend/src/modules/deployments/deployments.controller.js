...
import * as protectionService from "./protection.service.js";
...
export async function createDeployment(req, res) {
    try {
        ...
        const maxDeployments = maximums?.maximum_deployments || 0;
 ...
 * Executors get the protected stub script (not the raw script).
 */
export async function serveDeployment(req, res) {
    const { deployKey } = req.params;

    try {
        const cacheKey = `deployment:${deployKey}`;
        let deployment = await getCache(cacheKey);

        if (!deployment) {
            deployment = await deploymentsService.getByDeployKey(deployKey);
            if (deployment) {
                // Cache for 60 seconds to absorb spikes without overloading DB
        ...
        const isHttpClient = ua.includes("curl") || ua.includes("postman") || ua.includes("insomnia") ||
            ...
            ua.includes("undici") || ua.includes("okhttp") || ua.includes("java");

        if (isBrowser || isHttpClient) {
...
<body>
    ...
    <script>
        ...
        // Non-browser (executor) — serve the protected stub, NOT the raw script
        ...
        // Build the API base URL for the stub
        const protocol = req.headers['x-forwarded-proto'] || req.protocol;
        const host = req.headers['x-forwarded-host'] || req.headers['host'];
        const apiBaseUrl = `${protocol}://${host}`;

        // Generate the protected stub
        const stub = protectionService.generateStub(deployKey, apiBaseUrl);
...
/**
 * Challenge endpoint — issues a time-limited HMAC token for HWID verification.
 * GET /v1/challenge?key=<deployKey>&hwid=<hwid>
 */
export async function challengeDeployment(req, res) {
    ...
    try {
        // Verify deployment exists (use cache)
        const cacheKey = `deployment:${key}`;
        let deployment = await getCache(cacheKey);

        if (!deployment) {
            deployment = await deploymentsService.getByDeployKey(key);
            if (deployment) {
                await setCache(cacheKey, deployment, 60);
            }
        }
        ...
        // Generate timestamp + HMAC token
        const ts = Math.floor(Date.now() / 1000).toString();
        const token = protectionService.generateToken(key, hwid, ts);

        res.json({ ts, token });
    } catch (error) {
        logger.error("Challenge Error: %o", error);
...
export async function verifyDeployment(req, res) {
    const { key, hwid, ts, token } = req.query;

    if (!key || !hwid || !ts || !token) {
        return res.status(400).json({ error: "Missing parameters" });
    }

    try {
        // Verify the token
        const result = protectionService.verifyToken(key, hwid, ts, token, 60);
        if (!result.valid) {
