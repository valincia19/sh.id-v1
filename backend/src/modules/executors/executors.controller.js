...
export const updateExecutor = async (req, res) => {
    try {
        ...
        // ── Ownership check ──────────────────────────────────────────
        const existing = await executorService.getExecutorById(id);
        if (!existing) {
            return res.status(404).json({ error: "NotFound", message: "Executor not found" });
        }
        if (existing.owner_id !== userId) {
            return res.status(403).json({ error: "Forbidden", message: "You don't have permission to edit this executor." });
        }
        // ─────────────────────────────────────────────────────────────
