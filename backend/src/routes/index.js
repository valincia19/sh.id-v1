...
/**
 * Platform stats (public) — cached in Redis for 60s
 * GET /api/stats
 */
router.get('/stats', async (req, res) => {
  try {
    // Try Redis cache first (avoids 5 COUNT/SUM full-table scans)
    const cached = await getCache('cache:stats');
    if (cached) {
    ...
    await setCache('cache:stats', data, 60);
