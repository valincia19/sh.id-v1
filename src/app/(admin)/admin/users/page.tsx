...
// ── Fixed-position actions menu (same pattern as scripts) ──────────
function UserRowMenu({ user, isOpen, onToggle, onAction, onClose, btnRef }: {
    user: AdminUser;
    isOpen: boolean;
    ...
    const act = (a: string) => { onAction(a); onClose(); };
                ...
                <>
                    ...
                    >
                        <button onClick={() => act("edit")}
                            className="w-full flex items-center gap-2.5 px-3 py-2 text-[12px] font-mono text-offgray-200 hover:text-white hover:bg-white/[0.04] transition-colors">
                        ...
                        {user.account_status === "active" ? (
                            <button onClick={() => act("suspend")}
                                className="w-full flex items-center gap-2.5 px-3 py-2 text-[12px] font-mono text-yellow-500 hover:text-yellow-400 hover:bg-white/[0.04] transition-colors">
                                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><line x1="10" y1="15" x2="10" y2="9" /><line x1="14" y1="15" x2="14" y2="9" /></svg>
                                Suspend Account
                            </button>
                        ) : (
...
export default function AdminUsersPage() {
    ...
    const rowBtnRefs = useRef<Record<string, React.RefObject<HTMLButtonElement>>>({});
                        ...
                        <table className="w-full text-[13px]">
                            ...
                            <tbody className="divide-y divide-white/[0.03]">
                                {isLoading ? (
                                    ...
                                    users.map((u) => (
                                        <tr key={u.id} className="hover:bg-white/[0.01] transition-colors" onClick={() => setOpenMenuId(null)}>
                                            ...
                                            <td className="px-4 py-2" onClick={e => e.stopPropagation()}>
                                                {(() => {
                                                    if (!rowBtnRefs.current[u.id]) rowBtnRefs.current[u.id] = { current: null } as any;
                                                    return (
                                                        <UserRowMenu
                                                            user={u}
                                                            isOpen={openMenuId === u.id}
                                                            onToggle={(e) => { e.stopPropagation(); setOpenMenuId(openMenuId === u.id ? null : u.id); }}
                                                            onAction={(a) => handleAction(u, a)}
                                                            onClose={() => setOpenMenuId(null)}
                                                            btnRef={rowBtnRefs.current[u.id]}
                                                        />
                                                    );
                                                })()}
                                            </td>
