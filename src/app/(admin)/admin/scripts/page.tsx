...
function ScriptRowMenu({ script, isOpen, onToggle, onStatusChange, onDelete, onRestore, onClose }: {
    script: AdminScript;
    isOpen: boolean;
            ...
            {isOpen && (
                <>
                    ...
                    >
                        ...
                        {(script as any).deleted_at && (
                            <button onClick={() => act(() => onRestore(script.id))}
                                className="w-full flex items-center gap-2.5 px-3 py-2 text-[12px] font-mono text-emerald-400 hover:text-emerald-300 hover:bg-white/[0.04] transition-colors">
                                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export default function AdminScriptsPage() {
    ...
    const handleDelete = async (script: AdminScript) => {
        ...
        try {
            ...
            if (activeTab === "deleted") {
                // Already in deleted tab, shouldn't really happen but for consistency
                loadScripts(search, activeTab);
            } else {
    ...
    const handleRestore = async (id: string) => {
        try {
            await apiClient.patch(`/admin/scripts/${id}/restore`);
            setScripts(prev => prev.filter(s => s.id !== id));
            setTotal(prev => prev - 1);
            alert("Script restored successfully.");
        } catch { alert("Failed to restore script."); }
    };
                            ...
                            {isLoading ? (
                                ...
                                scripts.map(s => (
                                    <tr key={s.id} className="hover:bg-white/[0.01] transition-colors" onClick={() => setOpenMenuId(null)}>
                                        ...
                                        <td className="px-4 py-3 whitespace-nowrap">
                                            <span className={`text-[10px] font-mono font-bold px-1.5 py-0.5 rounded border ${statusColor(s.status, !!(s as any).deleted_at)}`}>
                                                {(s as any).deleted_at ? "deleted" : s.status}
                                            </span>
                                        </td>
