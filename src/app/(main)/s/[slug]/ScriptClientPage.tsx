...
const formatYoutubeEmbedUrl = (url: string) => {
    ...
    if (url.includes('youtube.com/watch?v=')) {
        embedUrl = url.replace('youtube.com/watch?v=', 'youtube-nocookie.com/embed/').split('&')[0];
    } else if (url.includes('youtu.be/')) {
        embedUrl = url.replace('youtu.be/', 'youtube-nocookie.com/embed/');
    }

    // Check if we converted to embed URL successfully, then append params
    if (embedUrl.includes('/embed/')) {
        const separator = embedUrl.includes('?') ? '&' : '?';
        embedUrl += `${separator}rel=0&modestbranding=1`;
    }
    return embedUrl;
};
        ...
        <div className="space-y-5 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
                ...
                <div className="lg:col-span-8 space-y-4">
                    ...
                    {script.youtubeUrl && (
                        <div className="relative aspect-video rounded-xl overflow-hidden border border-white/[0.06] bg-[#0d0f11] mt-4">
                            <iframe
                                width="100%"
                                height="100%"
                                src={formatYoutubeEmbedUrl(script.youtubeUrl)}
