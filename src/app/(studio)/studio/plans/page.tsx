...
export default function PlansPage() {
    ...
    const plans = [
        ...
        },
        {
            id: "pro",
            name: "Pro",
            price: { USD: "$15", IDR: "Rp 235.000" },
            period: "/month",
            description: "Power tools for serious developers",
            features: [
                "Everything in Free",
                "Deployments (1,000/mo)",
                "Key System (5,000/mo)",
                "Obfuscation (50/mo)",
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="space-y-1">
                    <h1 className="text-2xl md:text-3xl font-serif tracking-tight text-white">Subscription Plans</h1>
                    <p className="text-sm font-mono text-offgray-500 max-w-lg mt-2">Upgrade your studio to unlock more features and monetization tools.</p>
                </div>

                {/* Currency Toggle */}
                <div className="flex items-center bg-[#0a0c10] border border-white/[0.04] rounded-lg p-1 w-fit">
                    <button
                        onClick={() => setCurrency("IDR")}
                        className={`text-xs px-4 py-1.5 rounded-md transition-all font-mono uppercase tracking-widest ${currency === "IDR"
                    ...
                    <button
                        ...
                        className={`text-xs px-4 py-1.5 rounded-md transition-all font-mono uppercase tracking-widest ${currency === "USD"
                            ? "bg-white/[0.08] text-white shadow-sm"
                            : "text-offgray-500 hover:text-offgray-300"
                            }`}
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                {plans.map((plan) => {
                    ...
                    return (
                        ...
                        >
                            {isCurrent && (
                                <div className={`absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full border text-[10px] font-mono tracking-widest uppercase backdrop-blur-md flex items-center gap-1.5 whitespace-nowrap ${(plan.id === "custom" || getCountdown(data?.plan.expires_at || null) === "Expired")
                            ...
                            <div className="flex items-baseline mb-6">
                                <span className={`font-serif tracking-tight text-offgray-50 ${currency === 'IDR' && plan.id !== 'custom' && plan.id !== 'free' ? 'text-2xl' : 'text-4xl'}`}>
                                    {plan.price[currency]}
                                </span>
                            ...
                            <ul className="space-y-3 mb-8 flex-1">
                                {plan.features.map((feature) => (
                                    <li key={feature} className="flex items-start gap-3 text-xs text-offgray-400">
                                        <svg className={`w-4 h-4 shrink-0 mt-0.5 ${isCurrent ? (plan.id === "custom" || getCountdown(data?.plan.expires_at || null) === "Expired") ? "text-rose-400" : "text-emerald-400" : "text-offgray-600"}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                            ...
                            <button
                                disabled={plan.disabled || (isCurrent && getCountdown(data?.plan.expires_at || null) !== "Expired")}
                                className={`w-full h-11 rounded-md text-[10px] font-mono uppercase tracking-widest transition-all ${isCurrent
                                    ? (plan.id === "custom" || getCountdown(data?.plan.expires_at || null) === "Expired")
                            ...
                            >
                                {isCurrent
                                    ? getCountdown(data?.plan.expires_at || null) === "Expired"
                                        ? "Expired"
                                        : "Active"
                                    : plan.cta}
                            </button>
