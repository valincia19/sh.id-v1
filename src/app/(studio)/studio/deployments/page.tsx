        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-20">
            ...
            <section className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                    <div className="relative" ref={dropdownRef}>
                        <button
                            onClick={() => {
                                if (activeCount >= maxDeployments) return;
                                setDropdownOpen(!dropdownOpen);
                            }}
                            disabled={activeCount >= maxDeployments}
                            className={`group relative inline-flex items-center justify-center gap-2 h-9 px-4 rounded-md text-xs font-medium transition-all shadow-lg overflow-hidden ${activeCount >= maxDeployments
                                ? 'bg-white/[0.04] border border-white/[0.08] text-offgray-600 cursor-not-allowed opacity-50'
                                : 'bg-[#059669] hover:bg-[#10B981] text-white shadow-emerald-500/10 hover:shadow-emerald-500/25'}`}
                        >
                            {!loading && activeCount >= maxDeployments ? (
                                <>
                                    ...
                                    <span className="relative z-10">Quota Reached</span>
                                </>
                            ) : (
                                <>
            ...
            <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard
                    ...
                    value={loading ? "-" : String(maxDeployments)}
                    ...
                    color="emerald"
                    sub1={<span className="text-emerald-600">{activeCount} used</span>}
                    sub2={`${Math.max(0, maxDeployments - activeCount)} available`}
                    progress={maxDeployments > 0 ? Math.min(100, (activeCount / maxDeployments) * 100) : 0}
                />
                <StatCard
                    label="Active Files"
                ...
                <StatCard
                    label="Total Size"
                    value={loading ? "-" : formatBytes(stats?.total_size ?? 0)}
                    ...
                    color="offgray"
                    sub1="Across all files"
                />
                <StatCard
                    label="CDN Requests"
                    value={loading ? "-" : String(stats?.cdn_requests ?? 0)}
                            ...
                            {loading ? (
                                ...
                                <tr><td colSpan={5} className="px-5 py-12 text-center">
                                    <div className="flex flex-col items-center gap-3">
                                        ...
                                        <h2 className="text-lg font-serif tracking-tight text-offgray-100 mb-1.5">No deployments found</h2>
                                        <p className="text-sm font-mono text-offgray-500 max-w-sm mx-auto mb-6">Click &quot;New Deployment&quot; to upload your first file.</p>
                                    </div>
                                </td></tr>
                                ...
                                >
                                    <td className="px-3 py-2.5 whitespace-nowrap">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex flex-col">
                                                <span className="text-[14px] font-medium text-offgray-100 group-hover/row:text-emerald-300 transition-colors">
                                                    {file.title}
                                                </span>
                                                <span className="text-[11px] font-mono text-offgray-500 truncate max-w-[200px]" title={file.deploy_key}>
                                                    {file.deploy_key}
                                                </span>
                                            </div>
                                        </div>
                                    ...
                                    <td className="px-3 py-2.5 whitespace-nowrap">
                                        <span className="text-[13px] font-mono text-offgray-400">{formatBytes(file.file_size)}</span>
                                    </td>
                    ...
                    <div className="relative w-full max-w-sm bg-[#0a0c10] border border-white/[0.08] rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
                        ...
                        <div className="px-6 py-4 bg-white/[0.01] border-t border-white/[0.04] flex gap-3">
                            ...
                            <button
                                ...
                                className="flex-1 h-10 rounded-lg bg-[#059669] hover:bg-[#10B981] text-white text-xs font-medium transition-all shadow-lg shadow-emerald-500/20 disabled:opacity-30 disabled:cursor-not-allowed"
                            >
                                Create &amp; Open Editor
                            </button>
                        </div>
