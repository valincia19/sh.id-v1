        ...
        <div className="space-y-8 animate-in fade-in duration-700 pb-20 max-w-4xl mx-auto">
            ...
            <section className="flex flex-col gap-4">
                ...
                <div className="space-y-1.5">
                    ...
                    <p className="text-sm font-mono text-offgray-500 max-w-lg mt-2">
                        Deploy your static assets, images, or script files to our CDN.
                    </p>
                </div>
            </section>

            {/* Limit Warning - Compact & Consistent */}
            {!loadingData && availableSlots <= 0 && (
                <section className="relative group animate-in slide-in-from-top-4 duration-500">
                    <div className="relative p-5 bg-[#0a0c10] border border-rose-500/20 rounded-xl overflow-hidden flex items-center justify-between gap-4 shadow-xl">
                        <div className="flex items-center gap-4 relative z-10">
                            ...
                            <div className="space-y-0.5">
                                <h4 className="text-sm font-semibold text-rose-200 flex items-center gap-2">
                                    Quota Limit Reached
                                    <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-rose-500/10 border border-rose-500/20 text-rose-400 uppercase tracking-wider">Locked</span>
                                </h4>
                                <p className="text-[11px] font-mono text-offgray-500">
                                    Used all <span className="text-rose-400 font-bold">{maxDeployments}</span> slots. Please remove files or upgrade.
                                </p>
                            </div>
                        </div>
                        ...
                        <Link
                            href="/studio/plans"
                            className="h-8 px-4 rounded-lg bg-rose-500 text-white text-[10px] font-mono font-bold hover:bg-rose-400 transition-all shadow-lg shadow-rose-500/10 flex items-center shrink-0 uppercase tracking-widest"
                        >
                            Upgrade Plan
                        </Link>
                    </div>
            ...
            <section
                onDragOver={(e) => availableSlots > 0 && handleDragOver(e)}
                onDragLeave={handleDragLeave}
                onDrop={(e) => availableSlots > 0 && handleDrop(e)}
                onClick={() => availableSlots > 0 && fileInputRef.current?.click()}
                className={`
                    ...
                    ${availableSlots <= 0
                        ...
                        : isDragging
                            ? 'border-[#10B981] bg-[#10B981]/5 shadow-[0_0_40px_rgba(16,185,129,0.05)] cursor-pointer'
                            : 'border-white/[0.06] bg-[#0a0c10] hover:border-white/[0.12] hover:bg-white/[0.01] cursor-pointer'
                    }
                `}
            >
                <input
                ...
                <div className={`
                    w-12 h-12 rounded-xl transition-all duration-300 relative z-10 flex items-center justify-center mb-4
                    ${availableSlots <= 0
                        ? 'bg-rose-500/5 border border-rose-500/10 text-rose-500/30'
                        : `bg-[#11141B] border border-white/[0.06] group-hover:scale-105 
                           ${isDragging ? 'border-[#10B981]/50 text-[#10B981]' : 'text-offgray-600 group-hover:text-offgray-300'}
                          `
                    }
                `}>
                    {availableSlots <= 0 ? (
                ...
                <div className="text-center space-y-1 relative z-10">
                    <h2 className={`text-sm font-semibold tracking-tight ${availableSlots <= 0 ? 'text-offgray-600' : 'text-offgray-100 group-hover:text-white'}`}>
                        {availableSlots <= 0 ? 'Uploads Locked' : isDragging ? 'Drop Files Now' : 'Click to upload files'}
                    </h2>
                    <p className={`text-[11px] font-mono transition-colors duration-300 ${availableSlots <= 0 ? 'text-offgray-800' : 'text-offgray-500'}`}>
                        {availableSlots <= 0 ? 'Quota exceeded' : '.lua, .luau, or .txt (Max 50MB)'}
                    </p>
                </div>
            </section>
                ...
                <section className="space-y-4 animate-in slide-in-from-bottom-4 duration-500">
                    <div className="flex items-center justify-between">
                        ...
                        {files.some(f => f.status === 'idle' || f.status === 'error') && (
                            <button
                                onClick={startUpload}
                                className="h-8 px-4 rounded-lg bg-[#059669] hover:bg-[#10B981] text-[11px] font-medium text-white transition-all shadow-lg shadow-emerald-500/10"
                            >
                                Start Upload
                            </button>
                        )}
