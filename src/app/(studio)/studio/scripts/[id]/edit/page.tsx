...
export default function StudioScriptEditPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const [scriptData, hubsData, deploymentsData] = await Promise.all([
                ...
                // Detect key system mode from existing URL
                const existingKeyUrl = scriptData.keySystemUrl || '';
                const isBuiltinKey = existingKeyUrl.includes('getfreekey.scripthub.id') || existingKeyUrl.includes('getfreekey.localhost');

                // Detect loader mode from existing URL
                const existingLoaderUrl = scriptData.loaderUrl || '';
                const cdnBaseUrl = envConfig.cdnBaseUrl;
                const matchingDep = (deploymentsData.deployments || []).find((d: Deployment) =>
                    existingLoaderUrl.includes(d.deploy_key)
                );
                const isDeploymentLoader = !!matchingDep;
                ...
                setInitialData({
                    ...
                    keySystemMode: isBuiltinKey ? 'builtin' : 'manual',
                    loaderMode: isDeploymentLoader ? 'deployment' : 'custom',
                    selectedDeploymentId: matchingDep?.id || '',
                    thumbnailPreview: scriptData.thumbnailUrl ? getStorageUrl(scriptData.thumbnailUrl) : null,
                    gamePreview,
                    status: scriptData.status || 'draft',
                    youtubeUrl: scriptData.youtubeUrl || "",
                });
