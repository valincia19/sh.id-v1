...
export const mapScriptData = (data: any): Script => ({
    ...
    youtubeUrl: data.youtube_url || data.youtubeUrl,
 ...
 */
export const updateScriptStatus = async (id: string, status: 'published' | 'draft' | 'under_review'): Promise<Script> => {
    try {
        const response = await apiClient.patch(`/scripts/${id}`, { status });
        return mapScriptData(response.data.data);
    } catch (error) {
        throw handleApiError(error);
    }
 ...
 */
export const getTrendingScripts = async (
    filters: { period?: 'day' | 'week' | 'month' | 'all' } = {}
): Promise<Script[]> => {
    try {
        const response = await apiClient.get('/scripts/trending', { params: filters });
        return response.data.data.map(mapScriptData);
    } catch (error) {
        throw handleApiError(error);
    }
