...
export function Header() {
  ...
  const handleLogout = async () => {
    ...
    window.dispatchEvent(new Event('auth-change')); // Notify global auth context
    window.location.href = "/";
  };

  return (
    <>
      <header className="sticky top-0 z-30 w-full h-14 shrink-0 flex items-center border-b border-white/[0.06] bg-[#0a0c0f]/80 backdrop-blur-xl">
        <nav
          aria-label="Main navigation"
          className="w-full mx-auto max-w-[1148px] px-4 flex items-center justify-between gap-2.5"
        >
          {/* Brand */}
