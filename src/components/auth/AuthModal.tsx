  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err && typeof err === "object" && "code" in err) {
        const apiError = err as { code?: string; message?: string; remainingTime?: number };
        if (apiError.code === "ACCOUNT_LOCKED") {
          const minutes = apiError.remainingTime
            ? Math.ceil(apiError.remainingTime / 60)
            : 15;
      ...
      if (err && typeof err === "object" && "remainingAttempts" in err) {
        const apiError = err as { message?: string; remainingAttempts?: number };
        const attempts = apiError.remainingAttempts;
        if (attempts && attempts <= 2) {
          setError(`${apiError.message || "Invalid credentials"} (${attempts} attempt${attempts !== 1 ? 's' : ''} remaining before lockout)`);
          return;
        }
      }
